...
=== I am running in Windows, and when executing mvn spring-boot:run, I get the error "CreateProcess error=206, The filename or extension is too long."

This is a known issue with Windows and Spring Boot. For more information, see https://github.com/spring-projects/spring-boot/issues/17766.

There are a few ways this can be handled.

- For simply getting the app running, the easiest alternative is to run the compiled jar via `java -jar .\target\ms-tutorial-getting-started.jar`. To enable debugging, run with
[source]
----
...
- A work-around to enable running with `mvn spring-boot:run` is to soft link your m2 directory to the root of the drive.
[source]
----
