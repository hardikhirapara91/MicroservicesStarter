...
----
+
The above command starts the creation process of all the supporting services.
...
----
+
Running the above should produce something like below when everything is up and running:
+
[source,shell script]
----
...
- By default, this project is configured to utilize the spring boot liquibase property `drop-first`
in order to ease initial evaluation purposes and support rapid evaluation changes.

[TIP]
====
More details on `spring.liquibase.drop-first` can be found here: https://docs.spring.io/spring-boot/docs/current/reference/html/appendix-application-properties.html#data-migration-properties
====

The backend API has it defined for each service in `application-default.yml`
[source,yaml]
----
broadleaf:
  service_abc:
...
The `auth` supporting service docker image passes in the environment property:
[source,yaml]
----
auth:
  environment:
